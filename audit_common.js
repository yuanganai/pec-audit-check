/* PEC-Audit 模擬稽核點檢站 共用引擎 v1.0
   ─────────────────────────────────────────────────────────────
   離線優先：勾選／缺失／場次 全部先寫 localStorage（aud_state），
   照片壓縮後暫存 IndexedDB（aud_photos），有網路時自動上傳 Drive + 同步雲端。
   雲端：pec_kpi_store.gs  ?what=audit 讀 / action:'auditsave' 合併寫（伺服器端依 ts 新者勝）。
   頁面先定義 window.SEED = {remote, seed}，再載入本檔。 */
(function (w) {
  'use strict';
  var SEED = w.SEED || {};
  var REMOTE = SEED.remote || null;
  var BANK = SEED.seed || { sections: [], subs: [], items: [], history: [] };
  var LS_STATE = 'aud_state_v1', LS_LANG = 'aud_lang', LS_BY = 'aud_by', LS_CUR = 'aud_cur';
  ...
  function db() {
    ...
    dbP = new Promise(function (res, rej) {
      var rq = indexedDB.open('aud_photos', 1);
  ...
  /* ── 題庫 ── */
  function sections(type) { return BANK.sections.filter(function (s) { return !type || s.type === type; }); }
  function subs(sec) { return BANK.subs.filter(function (s) { return s.section === sec; }); }
  function items(type) {
    var all = BANK.items.concat(S.custom.items.filter(function (i) { return !i.del; }));
    return all.filter(function (i) { return (!type || i.type === type) && !(S.custom.hidden[i.id] && S.custom.hidden[i.id].h); });
  }
  ...
  function secName(id) { for (var i = 0; i < BANK.sections.length; i++) if (BANK.sections[i].id === id) return t(BANK.sections[i].zh, BANK.sections[i].en); return id; }
  var histMap = null;
  function histBy(id) {
    if (!histMap) { histMap = {}; BANK.history.forEach(function (h) { if (!h.item) return; (histMap[h.item] = histMap[h.item] || []).push(h); }); }
    return histMap[id] || [];
  }
  ...
  function setResult(sid, iid, r, note) {
    if (!S.results[sid]) S.results[sid] = {};
    var o = S.results[sid][iid] || {};
  ...
  function newFinding(sid, iid) {
    var it = item(iid);
    var f = { id: uid('f'), sid: sid, item: iid, type: it ? it.type : 'qsa', section: it ? it.section : '', desc: '', sev: 'Minor', dept: '', owner: '', due: '', photos: [], by: by(), ts: now(), created: today() };
  ...
  function progress(sid, type) {
    var its = items(type), R = S.results[sid] || {}, c = { total: its.length, ok: 0, ng: 0, na: 0, done: 0 };
    its.forEach(function (i) { var r = R[i.id]; if (r && r.r) { c.done++; c[r.r] = (c[r.r] || 0) + 1; } });
  ...
  w.AUD = { $: $, esc: esc, uid: uid, now: now, today: today, fmtTs: fmtTs, toast: toast, status: status,
    ...
    sections: sections, subs: subs, items: items, item: item, secName: secName, histBy: histBy, history: history, BANK: BANK, REMOTE: REMOTE,
